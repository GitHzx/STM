...
class Encoder_M(nn.Module):
    def __init__(self):
        ...
        self.res4 = resnet.layer3 # 1/8, 1024  ?????? 1/16, 1024
    ...
    def forward(self, in_f, in_m, in_o):
        ...
        r4 = self.res4(r3) # 1/8, 1024  ?????? 1/16, 1024
...
class STM(nn.Module):
    ...
    def memorize(self, frame, masks, num_objects): 
        ...
        (frame, masks), pad = pad_divide_by([frame, masks], 16, (frame.size()[2], frame.size()[3])) # 对图像帧和mask进行填充，要求被16整除

        # make batch arg list
        B_list = {'f':[], 'm':[], 'o':[]}
        for o in range(1, num_objects+1): # 1 - no 把该帧中的每个目标都添加进列表f m o
            B_list['f'].append(frame) # 添加该帧图像
            B_list['m'].append(masks[:,o]) # 添加该帧第o个目标mask
            # masks[:,1:o]：该帧从第一个目标到第o个目标mask
            # masks[:,o+1:num_objects+1]：该帧从第o+1个目标到最后一个目标mask
            # 对两个形状不同的Tensor按元素运算时，可能会触发⼴播（broadcasting）机制
            # torch.sum 对输入tensor的某一维度dim求和
            # tensor.clamp 对tensor进行上下限除去操作
            B_list['o'].append( (torch.sum(masks[:,1:o], dim=1) + torch.sum(masks[:,o+1:num_objects+1], dim=1)).clamp(0,1) )     

        # make Batch
        # 把字典中的list元素转换为tensor类型，例：
        # {'f': [tensor([1, 1, 1])], 'm': [tensor([2, 2, 2])], 'o': [tensor([3, 3, 3])]}
        # {'f': tensor([1, 1, 1]), 'm': tensor([2, 2, 2]), 'o': tensor([3, 3, 3])}
        B_ = {}
